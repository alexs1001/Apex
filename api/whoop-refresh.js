...
// Uses the stored refresh_token to get a new access_token from WHOOP.
// Called by the frontend when the access token is close to expiry.
export default async function handler(req, res) {
  ...
  if (!clientId || !clientSecret)
    return res.status(500).json({ error: 'server not configured (missing WHOOP env vars)' });
